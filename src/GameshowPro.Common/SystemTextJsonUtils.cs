...
public static class SystemTextJsonUtils
{
    ...
    public static async Task<T?> Depersist<T>(string? path, bool rethrowDeserializationExceptions, bool renameFailedFiles, ILogger? logger, CancellationToken? cancellationToken)
    {
        ...
        if (path is not null && File.Exists(path))
        {
            ...
            try
            {
                obj = await Depersist<T>(fileStream, cancellationToken);
    ...
    public static async Task<T> Depersist<T>(Stream stream, ILogger? logger, CancellationToken? cancellationToken)
    {
        T? obj;
        try
        {
            obj = await Depersist<T>(stream, cancellationToken);
    ...
    private static async Task<T?> Depersist<T>(Stream stream, CancellationToken? cancellationToken)
        => await JsonSerializer.DeserializeAsync<T>(stream, s_jsonSerializerOptions, cancellationToken ?? default);
    ...
    public static async Task Persist<T>(T obj, string? path, ILogger? logger, CancellationToken? cancellationToken)
    {
        ...
        await Persist<T>(obj, fileStream, cancellationToken);
    }
    ...
    /// <param name="obj">Object to be persisted.</param>
    public static async Task Persist<T>(T obj, Stream stream, CancellationToken? cancellationToken)
        => await JsonSerializer.SerializeAsync(stream, obj, s_jsonSerializerOptions, cancellationToken ?? default);
