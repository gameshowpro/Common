...
public abstract class MatchToBooleanConverter(object doNothing) : ICommonValueConverter, ICommonMultiValueConverter
{
    ...
    {
        return values[0] is int value0Int && values[1] is int value1Int
            ? BooleanToType(value0Int == value1Int, targetType)
            : BooleanToType((values[0] == null && values[1] == null) || values[0]?.Equals(values[1]) == true, targetType);
    }
    ...
    public object? Convert(object? value, Type targetType, object? parameter, CultureInfo culture)
    {
        ...
        {
            return (parameter is IEnumerable paramList && AnyElementMatches(value, paramList))
                    ||
                    parameter switch
                    {
